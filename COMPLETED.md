...
Java code rewritten against Mojang names: `ServerPlayerEntity`->`ServerPlayer`, `PlayerEntity`->`Player`, `ServerWorld`->`ServerLevel`, `Text`->`Component`, `Formatting`->`ChatFormatting`, `Vec3d`->`Vec3`, `StatusEffectInstance`->`MobEffectInstance`, `StatusEffects`->`MobEffects`, `Team`->`PlayerTeam`, `ScoreboardCriterion`->`ObjectiveCriteria`, `ScoreboardObjective`->`Objective`, `ScoreboardDisplaySlot`->`DisplaySlot`, `WorldSavePath.ROOT`->`LevelResource.ROOT`, `TitleS2CPacket`->`ClientboundSetTitleTextPacket`, `TitleFadeS2CPacket`->`ClientboundSetTitlesAnimationPacket`, `ActionResult`->`InteractionResult`, `CommandManager.literal`->`Commands.literal`. Method renames: `getUuid`->`getUUID`, `getPlayerOrThrow`->`getPlayerOrException`, `getPlayerManager().getPlayerList()`->`getPlayerList().getPlayers()`, `getHungerManager`->`getFoodData`, `setSaturationLevel`->`setSaturation`, `addStatusEffect`->`addEffect`, `removeStatusEffect`->`removeEffect`, `getEntityWorld`->`level()`, `getOverworld`->`overworld`, `getTopY(Heightmap.Type.X)`->`getHeight(Heightmap.Types.X)`, `requestTeleport`->`teleportTo`, `player.teleport(...)`->`player.teleportTo(level, x, y, z, Set<Relative>, yaw, pitch, resetCamera)`, `setVelocity`->`setDeltaMovement`, `isSneaking`->`isShiftKeyDown`, `Text.literal(...).formatted(F)`->`Component.literal(...).withStyle(F)`, `sendMessage(t, true)`->`sendSystemMessage(c, true)`, `sendFeedback`->`sendSuccess`, `getNullableObjective`->`getObjective`, `setObjectiveSlot`->`setDisplayObjective`, `getOrCreateScore(p, o).setScore(n)`->`getOrCreatePlayerScore(p, o).set(n)`, `getNameForScoreboard`->`getScoreboardName`, `getTeam`/`addTeam`/`removeTeam`->`getPlayerTeam`/`addPlayerTeam`/`removePlayerTeam`, `setPrefix`->`setPlayerPrefix`, `addScoreHolderToTeam`->`addPlayerToTeam`, `inventory.clear()`->`inventory.clearContent()`, `world.isClient()`->`world.isClientSide()`, `networkHandler.sendPacket`->`connection.send`. Unused imports from the commented-out boat/elytra/firework loadout were dropped.

Version bumped to **2.0.0**. `./gradlew build` is green; the JAR at `build/libs/kidsurvival-2.0.0.jar` declares the correct metadata. **Smoke testing on a real 26.1.2 Fabric server (and the release tag) is deferred to Phase 2.**
